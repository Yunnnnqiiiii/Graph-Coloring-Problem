	...
	while( T>90){
		...
		for(int i=0;i<500;i++){
			...
			for(int j=0;j<3 ;j++){
				...
				conflictNum2();
				//cout<<rand() % (N + 1) / (float)(N + 1)<<endl;
				//bool k=exp(-(conflictNum2()-minNum)/(T))==(rand()%100/(double)101);
				//cout<<k<<endl;
				//||exp(-(conflictNum2()-minNum)/T)>(rand() % (N + 1) / (float)(N + 1)) 
